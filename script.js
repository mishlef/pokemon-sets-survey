...
/* Sprites come straight from pokemondb.net's static "Home" sprite set — no
   fetching or caching needed, and it's the format pokemondb asks people to
   use when linking their images (an <a> back to the Pokedex page wrapping
   the <img>). See data.js for the sprite/dexSlug fields. */

function spriteUrl(slug) {
  return `https://img.pokemondb.net/sprites/home/normal/${slug}.png`;
}

function dexUrl(dexSlug) {
  return `https://pokemondb.net/pokedex/${dexSlug}`;
}
...
function buildCollapsedCard(p) {
  ...
  card.innerHTML = `
    <div class="card-top">
      <a href="${dexUrl(p.dexSlug)}" target="_blank" rel="noopener" onclick="event.stopPropagation()"><img class="sprite" src="${spriteUrl(p.sprite)}" alt="${p.name}" loading="lazy"></a>
...
function buildExpandedCard(p) {
  ...
  header.innerHTML = `
    <a href="${dexUrl(p.dexSlug)}" target="_blank" rel="noopener"><img class="sprite" src="${spriteUrl(p.sprite)}" alt="${p.name}"></a>
